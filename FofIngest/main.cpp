...
//    bool isDryRun;
    ...
    DBIngest::DBIngestor * fofIngestor;
    ...
    po::options_description progDesc("FofIngest - Ingest binary Fof catalogues into databases\n(Format must be the same as used by Stefan Gottloeber)\n\nFofIngest [OPTIONS] [dataFile]\n\nCommand line options:");
    ...
    FofSchemaMapper * thisSchemaMapper = new FofSchemaMapper(assertFac, convFac);     //registering the converter and asserter factories
    //FofSchemaMapper * thisSchemaMapper = new FofSchemaMapper();
    ...
    fofIngestor = new DBIngest::DBIngestor(thisSchema, thisReader, dbServer);
    fofIngestor->setUsrName(user);
    fofIngestor->setPasswd(pwd);
    
    //settings for different DBs (copy&paste from AsciiIngest)
    if(system.compare("mysql") == 0) {
        fofIngestor->setSocket(socket);
        fofIngestor->setPort(port);
        fofIngestor->setHost(host);
    } else if (system.compare("sqlite3") == 0) {
        fofIngestor->setHost(path);
    } else if (system.compare("unix_sqlsrv_odbc") == 0) {
        fofIngestor->setSocket("DRIVER=FreeTDS;TDS_Version=7.0;");
        //fofIngestor->setSocket("DRIVER=SQL Server Native Client 10.0;");
        fofIngestor->setPort(port);
        fofIngestor->setHost(host);
    ...
    fofIngestor->setPerformanceMeter(outputFreq);	// after how many lines should I print the status?
    fofIngestor->ingestData(bufferSize);  		// buffer size (in bytes??)
